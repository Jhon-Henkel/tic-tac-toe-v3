...
class EndGame
{
    ...
    {
        $gotOld = dataBase\DataBase::getDb()->query ("SELECT got_old FROM board WHERE id_board = 1");
        ...
        if ($endGame->setHaveWin(constants\config::STRING_X)) {
            $this->endGame = $gameOver.$congratulations.'<span class="x">X</span>'.$youWin;
            echo $this->endGame;
        } elseif ($endGame->setHaveWin(constants\config::STRING_O)) {
            $this->endGame = $gameOver.$congratulations.'<span class="o">O</span>'.$youWin;
            echo $this->endGame;
        }elseif ($resultsGotOld->got_old == constants\config::VALOR_DEU_VELHA)
        {
            $setEndGameGotOld = "UPDATE player SET X_O = false, IA = false WHERE id_player = 1";
            dataBase\DataBase::getDb()->query ($setEndGameGotOld);
    ...
    public function setHaveWin($play):bool
    {
        ...
        if (
            ...
            $this->haveWin = true;
            dataBase\DataBase::getDb()->query ($endGame);
        } else {
            $this->haveWin = false;
        }
        return($this->haveWin);
