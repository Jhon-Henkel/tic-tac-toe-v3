...
class IaPlay
{
    public function IaPlay (): void
    {
        ...
        $selectPlayer = dataBase\DataBase::getDb()->query ("SELECT IA, difficulty FROM player WHERE id_player = 1");
        ...
        if ($ResultPlayer->difficulty == VALOR_FACIL) {
            ...
            while ($ia == 1) {
                ...
                if (
                    ...
                    dataBase\DataBase::getDb()->query ($setIaFalse);
                }
            }
        }

        if ($ResultPlayer->difficulty == constants\config::VALOR_MEDIA && $ResultPlayer->IA == constants\config::VALOR_IA_ON)
        {
            $defend->defend();
        ...
        if ($ResultPlayer->difficulty == constants\config::VALOR_DIFICIL && $ResultPlayer->IA == constants\config::VALOR_IA_ON) {
            $attack->attack();
            $defend->defend();
            if ($ResultPlayer->IA == constants\config::VALOR_IA_ON) {
                for ($j = 0; $j <= 9; $j++) {
